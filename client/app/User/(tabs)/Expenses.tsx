      ...
      <ScrollView className="px-4">
        ...
        {filteredExpenses.map((exp) => (
          <View key={exp._id} className="bg-white p-4 mb-3 rounded-lg shadow">
            <Text className="text-gray-400 text-sm font-semibold">Expense Type</Text>
            <Text className="font-bold text-lg">{exp.type}</Text>
            <Text className="mt-1 text-gray-600">{exp.description || "Purpose"}</Text>
            <Text className="mt-1 text-gray-500 text-sm">{new Date(exp.createdAt).toLocaleDateString()}</Text>
            <View className="mt-2">
            ...
            <Text className="mt-2 font-bold">Rs. {exp.amount}</Text>
          </View>
        ))}
      </ScrollView>
    </View>
